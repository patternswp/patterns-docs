...
/* Local */
class PatternsDocsSettingsApi {
    constructor() {
        this.endpoint = 'patterns-docs/v1/settings';
    ...
    bindEvents() {
        document.addEventListener('click', (event) => {
            if (event.target.classList.contains('patterns-docs-gsn-close')) {
                this.modifySettings('hide_get_started_notice', true);
            }
            else if (event.target.classList.contains('patterns-docs-rn-permanent-close')) {
                this.modifySettings('remove_review_notice_permanently', true);
            }
            else if (event.target.classList.contains('patterns-docs-rn-temporary-close')) {
                this.modifySettings('remove_review_notice_temporary_date_time', 1);
            }
            else if (event.target.classList.contains('patterns-docs-install-plugins')) {
    ...
    updateUI() {
        if (this.settings.hide_get_started_notice) {
            const noticeElement = document.getElementById('patterns-docs-gsn');
            if (noticeElement) {
        ...
        if (!this.isLoadReview()) {
            const reviewNoticeElement = document.getElementById('patterns-docs-review-notice');
    ...
    async installPlugins() {
        const plugins = PatternsDocsLocalize.recommended_plugins;
        ...
        window.location.replace(PatternsDocsLocalize.theme_info_url);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    new PatternsDocsSettingsApi();
});
